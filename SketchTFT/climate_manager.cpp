...
void CM_Update_Leds(int tid){
  ...
  RTOS_Set_Task_Timer(tid, currentLedEffect.exec(leds, currentLedEffect.occs_away));
  
  //Serial.println(rtos_tasks[tid].exec_time);
...
void CM_Show_LedEffect(unsigned long (*exec)(CRGB* leds, unsigned char occ), unsigned char occs_amount){
...
void CM_Fog(int tid){
  ...
  if(!fogState) RTOS_Set_Task_Timer(tid, random(3, 11)*1000);
  else RTOS_Set_Task_Timer(tid, random(3, 6)*1000);
...
void CM_Pump(int tid){
  ...
  if(!pumpState) RTOS_Set_Task_Timer(tid, 10000);
  else RTOS_Set_Task_Timer(tid, 800);
  
  pumpState = !pumpState;
}

void CM_Update_Sensors(int tid){
  if(senEnPinState) digitalWrite(SEN_ENABLE_PIN, HIGH);
  else{
    dirtHumidity = analogRead(DIRT_HUMIDITY_PIN)/4;
    digitalWrite(SEN_ENABLE_PIN, LOW);
  }
  //Serial.println(senEnPinState);
  senEnPinState = !senEnPinState;
}
