...
unsigned char Encoder::tick(){
  ...
  Serial.print(pinLeftState);
  Serial.print(" | RGHT PIN: ");
  Serial.println(pinRightState);*/
...
void Encoder::tickButton(){
  ...
  if(lastButtonState != state && state == HIGH) buttonState = HIGH;
  else if(lastButtonState == state) buttonState = LOW;
  else if(lastButtonState != state && state == LOW) buttonState = LOW;
